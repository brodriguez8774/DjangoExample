...
class PizzaToppingRelationship(models.Model):
    ...
    def delete(self, *args, **kwargs):
        """
        Modify model delete behavior.
        """
        # On deletion, update Pizza model as well to ensure pricings stay consistent.
        related_pizza = self.pizza
        super(PizzaToppingRelationship, self).delete(*args, **kwargs)
        related_pizza.save()


@receiver(post_save, sender=PizzaToppingRelationship)
def save_related_pizza_post_save(sender, instance, **kwargs):
    """
    Upon saving a Pizza-Topping relationship, update the related Pizza model as well.
    This ensures Pizza pricings stay accurate.
    """
    instance.pizza.save()
...
class Pizza(models.Model):
    ...
    """
    # Relational fields.
    toppings = models.ManyToManyField('Topping', through='PizzaToppingRelationship', blank=True)
    # Model fields.
